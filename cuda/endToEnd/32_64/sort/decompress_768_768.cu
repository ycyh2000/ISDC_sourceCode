...
)
{
    ...
    for (int i = 0; i < (((K_GLOBAL * PROCESSLINE) / M) + 31) >> 5; i++)
    {
        ...
        for (int j = 0; j < (N / MANT_DECOMPRESS_NUM_PER); j++) {
            ...
            if (backID_ele != 0) {
                uint32_t packedDataTemp = scan_mant[5][32 + tid - 1] - scan_mant[5][32 + ((tid>>3)<<3) - 1];
            ...
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 3) * M) + packedIndexx] = packedDatax;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 3) * M) + packedIndexy] = packedDatay;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 3) * M) + packedIndexz] = packedDataz;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 3) * M) + packedIndexw] = packedDataw;
