        ...
        for (int j = 0; j < (N / MANT_DECOMPRESS_NUM_PER); j++) {
            ...
            if ((dataReadLoop >= 0) && (dataRegEmpty >= 32 * 32)) {
                ...
                if (currentDataSharedWriteAddr >= mantDataEndAddr) {
                    currentDataSharedWriteAddr = currentDataSharedWriteAddr - 31;
                }
            ...
            uint32_t currentMantGroupAddr = scan_width[5][32 + completed_group + current_group];  //已经处理的组数 + 当前正要处理第几组。
            ...
            if (backID_ele != 0) {
                uint32_t packedDataTemp = scan_mant[5][32 + tid - 1] - scan_mant[5][32 + ((tid >> 4) << 4) - 1];
...
//            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 4) * M) + packedIndexx] = packedDatax;
//            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 4) * M) + packedIndexy] = packedDatay;
//            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 4) * M) + packedIndexz] = packedDataz;
//            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 4) * M) + packedIndexw] = packedDataw;
