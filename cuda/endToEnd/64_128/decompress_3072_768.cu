...
)
{
    ...
    for (int i = 0; i < (((K_GLOBAL * PROCESSLINE) / M) + 31) >> 5; i++)
    {
        ...
        for (int j = 0; j < (N / MANT_DECOMPRESS_NUM_PER); j++) {
            ...
            //temp0 = packedDataw;
            ///* ---------------- Layer 1: stride = 1 ---------------- */
            //temp1 = ((tid & 15) == 0) ? 0 : scan_mant[0][32 + tid - 1];
            //temp0 = temp0 + temp1;
            //scan_mant[1][32 + tid] = temp0;
            ///* ---------------- Layer 2: stride = 2 ---------------- */
            //temp1 = ((tid & 15) < 2) ? 0 : scan_mant[1][32 + tid - 2];
            //temp0 = temp0 + temp1;
            //scan_mant[2][32 + tid] = temp0;
            ///* ---------------- Layer 3: stride = 4 ---------------- */
            //temp1 = ((tid & 15) < 4) ? 0 : scan_mant[2][32 + tid - 4];
            //temp0 = temp0 + temp1;
            //scan_mant[3][32 + tid] = temp0;
            ///* ---------------- Layer 4: stride = 8 ---------------- */
            //temp1 = ((tid & 15) < 8) ? 0 : scan_mant[3][32 + tid - 8];
            //temp0 = temp0 + temp1;
            //scan_mant[4][32 + tid] = temp0;
            ///* ---------------- Apply group prefix (group size = 16) ---------------- */
            //// lane id inside each 16-thread group: 0..15
            //const int lane16 = (tid & 15);
            //if (lane16 == 0) {
            //    // First thread in each 16-thread group: no carry-in
            //    // (keep packedDatax/y/z/w unchanged)
            //}
            //else {
            //    // Carry-in = prefix sum of previous thread inside the SAME 16-thread group
            //    uint32_t packedDataTemp = scan_mant[4][32 + tid - 1];
            //    packedDatax += packedDataTemp;
            //    packedDatay += packedDataTemp;
            //    packedDataz += packedDataTemp;
            //    packedDataw += packedDataTemp;
            //}
            ////endAddr = (currentDataSharedReadAddr + currentMantGroupAddr - 1 - backID * MANT_DECOMPRESS_NUM_PER * currentGroupResidualWidth);
