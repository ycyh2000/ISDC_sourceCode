...
// Each thread in the warp calls this function.
// Adjacent two threads (0&1, 2&3, ...) form a group.
// Within each group, the 8 (index, weight) pairs are sorted by index.
// After sorting, the smaller 4 indices go to the even lane (0,2,4,...),
// and the larger 4 go to the odd lane (1,3,5,...).
__device__ void warp_pairwise_sort_reg(
...
)
{
    ...
    const int tid = threadIdx.x & 31;          // warp lane 0..31
    const int group_id = tid / 2;              // 0..15
    const int lane_in_group = tid % 2;         // 0 (even) or 1 (odd)
    const int group_start = group_id * 8;      // start index in shared memory for this group
    const int base = group_start + lane_in_group * 4; // this thread's 4-element block

    __shared__ uint32_t s_idx[16 * 8]; // 16 groups * 8 elements = 128
    __shared__ uint32_t s_wgt[16 * 8];
    ...
    // Bitonic sort for 8 elements within the group
    const int N_GROUP = 8;
    for (int size = 2; size <= N_GROUP; size <<= 1) {
        for (int stride = size >> 1; stride > 0; stride >>= 1) {
            #pragma unroll
            for (int k = 0; k < 4; ++k) {
                int i = base + k;               // global shared memory index
                int j = i ^ stride;             // paired index
                // Ensure both indices belong to the same group
                if (i < j && i >= group_start && j < group_start + N_GROUP) {
    ...
    // Read back the sorted 4 elements for this thread
    #pragma unroll
    for (int k = 0; k < 4; ++k) {
        idx[k] = s_idx[base + k];
        wgt[k] = s_wgt[base + k];
    ...
    for (int i = 0; i < (((K_GLOBAL * PROCESSLINE) / M) + 31) >> 5; i++)
    {
        ...
        for (int j = 0; j < (N / MANT_DECOMPRESS_NUM_PER); j++) {
            ...
            uint32_t indexMemory = index_data_u32[
                bid * (K_GLOBAL_SPARSE / 4) + (i * (N / MANT_DECOMPRESS_NUM_PER) + j) * 32 + tid
            ];

            uint32_t packedIndexx = (indexMemory >> 0)  & 0xFF;
            uint32_t packedIndexy = (indexMemory >> 8)  & 0xFF;
            uint32_t packedIndexz = (indexMemory >> 16) & 0xFF;
            uint32_t packedIndexw = (indexMemory >> 24) & 0xFF;
            ...
            if (backID_ele != 0) {
                ...
                //uint32_t packedDataTemp = __shfl_up_sync(0xFFFFFFFF, packedDataw, 1);
            ...
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 1) * M) + packedIndexx] = packedDatax;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 1) * M) + packedIndexy] = packedDatay;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 1) * M) + packedIndexz] = packedDataz;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 1) * M) + packedIndexw] = packedDataw;
