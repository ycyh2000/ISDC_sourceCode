...
)
{
    ...
    for (int i = 0; i < (((K_GLOBAL * PROCESSLINE) / M) + 31) >> 5; i++)
    {
        ...
        for (int j = 0; j < (N / MANT_DECOMPRESS_NUM_PER); j++) {
            ...
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 2) * M) + packedIndexx] = packedDatax;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 2) * M) + packedIndexy] = packedDatay;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 2) * M) + packedIndexz] = packedDataz;
            (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * 8 + (tid >> 2) * M) + packedIndexw] = packedDataw;
