...
////            bid * (K_GLOBAL_SPARSE / 4) + i * 32 + tid
//            (bid * K_GLOBAL_SPARSE + i * 32 * 4 + tid * 4) >> 5
//        ];
//        bool packedSignx = (signMemory >> ((tid % 8) * 4 + 0))  & 0x1;
//        bool packedSigny = (signMemory >> ((tid % 8) * 4 + 1))  & 0x1;
//        bool packedSignz = (signMemory >> ((tid % 8) * 4 + 2))  & 0x1;
//        bool packedSignw = (signMemory >> ((tid % 8) * 4 + 3))  & 0x1;
...
//        if(bid == 0){
//            printf("tid = %d, expMemory = %u, packedExpx = %u, packedExpy = %u, packedExpz = %u, packedExpw = %u\n",tid, expMemory, packedExpx, packedExpy, packedExpz, packedExpw);
//        }
...
//            printf("ys1: index1 = %u, index2 = %u\n",
//                   (currentDataSharedWriteAddr >> 5) & MANT_MEMORY_MASK,
//                   (mantDataBaseAddr + currentDataSharedWriteAddr) >> 5);
//
//            printf("ys1: mantDataBaseAddr = %u, currentDataSharedWriteAddr = %u\n",
//                   mantDataBaseAddr, currentDataSharedWriteAddr);
//            printf("ys1: sum = %u, index2 = %u\n",
//                   mantDataBaseAddr,
//                   currentDataSharedWriteAddr);

//             mantDataMemory[(currentDataSharedWriteAddr >> 5) & MANT_MEMORY_MASK]  = 0;
...
//        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexx] = packedSignx;
//        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexy] = packedSigny;
//        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexz] = packedSignz;
//        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexw] = packedSignw;


        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexx] = ((uint16_t)packedSignx << 15) | ((uint16_t)packedExpx << 7) | (packedDatax & 0x7F);
        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexy] = ((uint16_t)packedSigny << 15) | ((uint16_t)packedExpy << 7) | (packedDatay & 0x7F);
        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexz] = ((uint16_t)packedSignz << 15) | ((uint16_t)packedExpz << 7) | (packedDataz & 0x7F);
        (decompressed_weight_gpu)[(K_GLOBAL * bid * PROCESSLINE + dataDecompressedRound * 32 * GROUPNUM * M + tid * M) + packedIndexw] = ((uint16_t)packedSignw << 15) | ((uint16_t)packedExpw << 7) | (packedDataw & 0x7F);
...
//    uint16_t* s_data = (uint16_t*)malloc(oc_size * ic_size * sizeof(uint16_t));
//    cudaMemcpy(s_data, d_decompressed_weight, oc_size * ic_size * sizeof(uint16_t), cudaMemcpyDeviceToHost);
//    for(int i = 0; i < 10; i++){
//        printf("d_decompressed_weight[%d] = %u\n", i, s_data[i]);
//    }
//    free(s_data);
...
//    printf("C received ptr: %p\n", d_startAddr);
//    printf("Current CUDA device: %d\n", dev);
//    uint32_t* a_data = (uint32_t*)malloc((oc_size + 1) * sizeof(uint32_t));
//    cudaMemcpy(a_data, d_startAddr, (oc_size + 1) * sizeof(uint32_t), cudaMemcpyDeviceToHost);
//    for(int i = 0; i < oc_size + 1; i++){
//        printf("d_compressed_weight_data[%d] = %u\n", i, a_data[i]);
//    }
//    free(a_data);
